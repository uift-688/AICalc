...
if weights_dict is None or bias_dict is None or "--NoModelSave" in sys.argv:
...
if __name__ == "__main__":
    # デモ用のデータ生成
    X = np.array([[1, 2], [3, 5], [4, 7], [2, 3]])
    y_plus = np.array([[3], [8], [11], [5]])
    y_minus = np.array([[0], [1], [3], [1]])
    ...
    while True:
        try:
            ...
            print("構文エラー: \"<int:左オペランド> <int:右オペランド> <select:+|->\"の形式で入力する必要があります。")
            continue  # 次のループに進む
        else:
